...
// Cache object: { videoId: { filePath, expiresAt } }
const videoCache = {};
...
app.get("/api/tiktok", async (req, res) => {
  ...
  if (token !== "my_super_secret_token_123") return res.status(403).json({ error: "⛔ Sai token" });
  if (!url) return res.status(400).json({ error: "❌ Thiếu URL TikTok" });

  try {
    // Lấy ID video từ link để cache
    ...
    if (cached && cached.expiresAt > Date.now() && fs.existsSync(cached.filePath)) {
      console.log("✅ Dùng video cache:", videoId);
    ...
    console.error("❌ Lỗi server/API:", err.response?.data || err.message);
